...
(function () {
    ...
    //    canvas منفصل فوق الـ game — يتمسح كل frame
    // ─────────────────────────────────────────────
    var _csActive    = false;
    var _csRaf       = null;
    var _csStart     = 0;
    var _csOvCanvas  = null;   // canvas الـ overlay المنفصل
    var _csOvCtx     = null;
    ...
    var CS_TIMELINE = [
        ...
        { t: 1080, end: 1320, frame: 'impact',    label: '☄️ METEOR PUNCH!!' },
    ...
    // أنشئ canvas overlay مرة واحدة فوق gameScreen
    function _ensureOverlayCanvas() {
        if (_csOvCanvas) return;
        var gs = document.getElementById('gameScreen');
        if (!gs) return;
        _csOvCanvas = document.createElement('canvas');
        _csOvCanvas.style.cssText =
            'position:absolute;top:0;left:0;width:100%;height:100%;' +
            'pointer-events:none;z-index:500;display:none;';
        gs.appendChild(_csOvCanvas);
        _csOvCtx = _csOvCanvas.getContext('2d');
    }

    // ── loop الـ cutscene — يمسح الـ overlay كل frame ──
    function _runCutscene() {
        if (!_csActive) return;

        var elapsed = Date.now() - _csStart;

        // مطابقة أبعاد الـ canvas مع الشاشة (لو اتغيّرت)
        var gs = document.getElementById('gameScreen');
        var W  = gs ? gs.offsetWidth  : window.innerWidth;
        var H  = gs ? gs.offsetHeight : window.innerHeight;
        if (_csOvCanvas.width !== W)  _csOvCanvas.width  = W;
        if (_csOvCanvas.height !== H) _csOvCanvas.height = H;

        var ctx = _csOvCtx;

        // ══ امسح كل الـ overlay كل frame — جوهر الإصلاح ══
        ctx.clearRect(0, 0, W, H);

        var phase  = _getCurrentPhase(elapsed);
        var cx     = (typeof p1 !== 'undefined' && p1) ? p1.x           : W * 0.25;
        var cy     = (typeof CONFIG !== 'undefined')   ? CONFIG.groundY  : H - 55;
        var facing = (typeof p1 !== 'undefined' && p1) ? (p1.facing||1)  : 1;

        // 1. طبقة تعتيم تدريجية
        var bgAlpha = Math.min(0.55, elapsed / 400 * 0.55);
        ctx.fillStyle = 'rgba(0,0,0,' + bgAlpha + ')';
        ctx.fillRect(0, 0, W, H);

        // 2. هالة نارية نابضة حول الشخصية
        var auraR = 85 + 10 * Math.sin(elapsed * 0.018);
        var grd   = ctx.createRadialGradient(cx, cy - 60, 8, cx, cy - 60, auraR);
        grd.addColorStop(0,   'rgba(255,160,0,0.55)');
        grd.addColorStop(0.5, 'rgba(255,80,0,0.25)');
        grd.addColorStop(1,   'transparent');
        ctx.save();
        ctx.fillStyle = grd;
        ctx.beginPath(); ctx.arc(cx, cy - 60, auraR, 0, Math.PI * 2); ctx.fill();
        ctx.restore();

        // 3. فريم الـ sheet (فريم واحد فقط لكل phase)
        if (_sheetReady) {
            _drawFrame(ctx, phase.frame, cx, cy, 130, facing);
        }

        // 4. جسيمات نارية مستقلة
        _tickCsParticles(ctx, cx, cy);

        // 5. نص المرحلة — fade in + scale in
        if (phase.label) {
            var tAlpha = Math.min(1, (elapsed - phase.t) / 120);
            var tScale = 0.6 + 0.4 * Math.min(1, (elapsed - phase.t) / 180);
            ctx.save();
            ...
            ctx.fillStyle    = phase.frame === 'impact' ? '#ffee00' : '#ff9900';
            ctx.strokeText(phase.label, 0, 0);
            ctx.fillText  (phase.label, 0, 0);
            ctx.restore();
        }

        // 6. Flash أبيض لحظي عند الـ Impact
        if (elapsed >= 1080 && elapsed < 1200) {
            var fAlpha = Math.max(0, 0.65 - (elapsed - 1080) / 120 * 0.65);
            ctx.fillStyle = 'rgba(255,100,0,' + fAlpha + ')';
            ctx.fillRect(0, 0, W, H);
        }
    ...
    // جسيمات مستقلة عن الـ game particles — تُرسم على overlay
    var _csParticles = [];
    function _tickCsParticles(ctx, cx, cy) {
        if (Math.random() < 0.5) {
    ...
    function _startCutscene() {
        ...
        // أنشئ/أظهر overlay canvas المنفصل
        _ensureOverlayCanvas();
        if (_csOvCanvas) _csOvCanvas.style.display = 'block';
    ...
    function _endCutscene() {
        ...
        // امسح وأخفي overlay canvas
        if (_csOvCtx && _csOvCanvas) {
            _csOvCtx.clearRect(0, 0, _csOvCanvas.width, _csOvCanvas.height);
            _csOvCanvas.style.display = 'none';
        }
        _csParticles = [];

        // أخفي الـ cinematic overlay
        var cs = document.getElementById('mpCutscene');
        if (cs) {
