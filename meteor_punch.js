...
(function () {
    ...
    // ─────────────────────────────────────────────────────────────
    var SHEET_SRC  = 'ult_sheet.png';   // 12 فريم × 619×547
    var FRAME_W    = 619;
    var FRAME_H    = 547;
    var FRAME_COUNT= 12;
    ...
    var Energy = {
        ...
        add: function (amount) {
            ...
            if (this.current >= this.max && !this.active) {
                ...
                this.full    = true;
                this._onFull();   // بس يوهج الزر — مش بيشغّل الـ ultimate
            }
        },
        ...
        // لما الـ energy تكتمل — بس يوهج الزر ويستنى الضغطة
        _onFull: function () {
            ...
            if (typeof damageNumbers !== 'undefined' &&
                ...
                damageNumbers.push({
                    x:p1.x, y:p1.y - 110,
                    vy:-2.2, alpha:1,
                    text:'☄️ ULTIMATE READY!',
                    color:'#ffcc00', size:20
                });
            }
            if (typeof ScreenFlash !== 'undefined')
                ScreenFlash.trigger('#ffaa00', 0.2);
        },

        // لما اللاعب يضغط الزر — تشتغل الـ ultimate فعلاً
        _onActivated: function () {
            ...
            if (typeof damageNumbers !== 'undefined' &&
                ...
                damageNumbers.push({
                    ...
                    vy:-2.5, alpha:1,
                    text:'☄️ ULTIMATE!',
                    color:'#ff4400', size:24
                });
            }
            var bar = document.getElementById('mpEnergyBar');
            var btn = document.getElementById('btnMeteor');
            if (bar) { bar.classList.remove('mp-energy-ready'); bar.classList.add('mp-ult-active'); }
            if (btn) { btn.classList.remove('mp-btn-ready');    btn.classList.add('mp-ult-active');  }
        },
    ...
    function _injectUI() {
        ...
        style.textContent = [
            ...
            '  font-size:1.6rem;transition:transform 0.08s;',
            '}',
            /* جاهز — ينتظر الضغط */
            '#btnMeteor.mp-btn-ready{',
            '  display:flex;',
            '  border-color:rgba(255,200,0,0.95);',
            '  animation:mpBtnReady 0.6s ease-in-out infinite alternate;',
            '}',
            '@keyframes mpBtnReady{',
            '  from{box-shadow:0 0 10px rgba(255,150,0,0.5);transform:scale(1);}',
            '  to  {box-shadow:0 0 24px rgba(255,200,0,0.95),0 0 48px rgba(255,80,0,0.45);transform:scale(1.1);}',
            '}',
            /* شغّال — الـ ultimate active */
            '#btnMeteor.mp-ult-active{',
            '  display:flex;',
            '  border-color:rgba(255,80,0,0.95);',
            '  animation:mpBtnUlt 0.35s ease-in-out infinite alternate;',
            '  cursor:default;',
            '}',
            '@keyframes mpBtnUlt{',
            '  from{box-shadow:0 0 14px rgba(255,80,0,0.6);transform:scale(1);}',
            '  to  {box-shadow:0 0 30px rgba(255,120,0,1),0 0 60px rgba(255,40,0,0.5);transform:scale(1.12);}',
            '}',
    ...
    // ── زر الـ Ultimate — يشتغل بالضغط بس لما الـ energy full ──
    function _bindUltButton() {
        var btn = document.getElementById('btnMeteor');
        if (!btn) return;

        function onPress(e) {
            e.preventDefault();
            if (!Energy.full || Energy.active) return;
            if (typeof gameActive !== 'undefined' && !gameActive) return;
            // شغّل الـ ultimate
            Energy._activate();
        }
    ...
    function _init() {
        ...
        setTimeout(function () {
            _hookGameLoop();
            _bindUltButton();   // بعد ما الـ DOM يكون جاهز
        }, 500);
    }
